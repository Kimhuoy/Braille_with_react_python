...
const TranslateFileBraille = ({ onClosePopup }) => {
  const { setResultFileBraille } = appStore();
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/updatebraille",
        formData,
        {
          headers: {
      ...
      const dataInFileBraille = response?.data?.uploaded;
      setResultFileBraille({ resultDataBraille, dataInFileBraille });
    } catch (error) {
...
export default TranslateFileBraille;
